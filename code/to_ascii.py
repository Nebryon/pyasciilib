...
La fonction array_to_ascii prend une liste de listes contenant la luminosité de chaque pixel (entre 0 et 255), une méthode de retour ("list" pour une liste de listes, "text" pour un texte directement imprimable), une taille (tuple = largeur, hauteur) pour l'image de sortie, qui est par défaut la même que celle d'entrée, ainsi qu'une liste modifiable de caractères ASCII à utiliser du plus au moins dense pour transformer l'image en ASCII.""",
...
La función array_to_ascii toma una lista de listas que contiene el brillo de cada píxel (entre 0 y 255), un método de retorno ("list" para una lista de listas, "text" para texto directamente imprimible), un tamaño (tupla = ancho, alto) para la imagen de salida, que por defecto es igual al de entrada, así como una lista modificable de caracteres ASCII opcional para usar de mayor a menor densidad para transformar la imagen en ASCII.""",
...
array_to_ascii 函数接受包含每个像素亮度的列表（0 到 255 之间），返回方法（“list”表示列表列表，“text”表示直接可打印的文本），输出图像的大小（元组=宽度，高度），默认为与输入相同，以及一个可选的可修改 ASCII 字符列表，用于从最密到最稀疏的字符来将图像转换为 ASCII。""",
...
array_to_ascii 関数は、各ピクセルの明るさ（0 から 255 の間）を含むリストを受け取り、戻り方法 ("list" はリストのリスト、"text" は直接印刷可能なテキスト)、出力画像のサイズ (タプル = 幅、高さ) を指定し、デフォルトでは入力と同じサイズになります。また、最も密から最も希薄なものまでの可変 ASCII 文字リスト (オプション) を使用して、画像を ASCII に変換します。""",
...
array_to_ascii 함수는 각 픽셀의 밝기 (0에서 255 사이)를 포함하는 리스트를 입력으로 받고, 반환 방법 ("list"는 리스트의 리스트, "text"는 직접 인쇄 가능한 텍스트), 출력 이미지의 크기 (튜플 = 너비, 높이)를 입력과 동일하게 기본 설정하며, 밀도 높은 순서에서 낮은 순서로 사용할 수 있는 수정 가능한 ASCII 문자 목록 (선택 사항)을 사용하여 이미지를 ASCII로 변환합니다.""",
...
تأخذ الدالة array_to_ascii قائمة من القوائم تحتوي على سطوع كل بكسل (بين 0 و 255)، وطريقة الإرجاع ("list" لقائمة القوائم، "text" لنص يمكن طباعته مباشرة)، وحجم الصورة الناتجة (tuple = العرض، الارتفاع)، والذي يكون بشكل افتراضي نفس حجم الإدخال، بالإضافة إلى قائمة اختيارية قابلة للتعديل من أحرف ASCII للاستخدام من الأكثر كثافة إلى الأقل لتحويل الصورة إلى ASCII.""",
    }
...
def image_to_ascii(path_to_image: str, returned: str, size: tuple = (0, 0), chars: list = ["@", "#", "%", "*", "+", "=", "-", ":", "."]):
    ...
    return array_to_ascii(image_array, returned, size, chars)




def array_to_ascii(image_array: list, returned: str, size: tuple = (0, 0), chars: list = ["@", "#", "%", "*", "+", "=", "-", ":", "."]):
    """
    This function takes as input : 
